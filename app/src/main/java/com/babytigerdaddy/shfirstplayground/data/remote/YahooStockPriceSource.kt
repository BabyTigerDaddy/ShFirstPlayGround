 ...
 *
 * 한국 종목 심볼: 코스피 `{code}.KS`, 코스닥 `{code}.KQ`. 시장을 모르니 .KS 먼저, 없으면 .KQ.
...
class YahooStockPriceSource @Inject constructor() : StockPriceSource {
    ...
    override suspend fun fetchPrice(code: String): Long? = withContext(Dispatchers.IO) {
        val clean = code.filter { it.isDigit() }
        if (clean.length < 6) return@withContext null
        for (suffix in SUFFIXES) {
            val price = tryFetch(clean + suffix)
